...
from CreateDatasetByGAN import GANNumericalLabel
...
def find_minimal_distance_IDS(target_label, train_IDs, labels, top_k):
    dic_dist = {}
    for ID in train_IDs:
        label = labels[ID, :]
        # print(label)
        dist = np.sqrt(np.sum(np.square(target_label-label)))
        dic_dist[ID] = dist
        # print(dist)
    ...
    top_k_min_IDs = []
    for i in sort_dist[:top_k]:
        top_k_min_IDs.append(i[0])
    return top_k_min_IDs
def create_CSI_by_GAN(minimal_IDs, labels, data_path_s):
    csi_gan = []
    # get the minimal_IDs and associated data
    gan_x = []
    for ID in minimal_IDs:
        X = np.load(data_path_s + "channel_measurement_" + str(ID).zfill(6) + '.npy')
        X_dims = np.reshape(X, (X.shape[0] * X.shape[1]))
        gan_x.append(X_dims)
    gan_x = np.array(gan_x)
    csi_gan = np.mean(gan_x, axis=0)
    # gan = GANNumericalLabel()
    # gan.train(gan_x)
    # noise = np.random.normal(0,1,(1,6400))
    # csi_gan = gan.generator.predict(noise)
    #print(csi_gan)
    return csi_gan
def save_csi_to_file(index_now, X_new, data_path_new, scenario):
    with open(data_path_new + "channel_measurements_" + scenario + "/" + "channel_measurement_" + str(index_now).zfill(6) + '.npy', 'wb') as f:
        np.save(f, X_new)
    # test = np.load(data_path_new + "channel_measurements_" + scenario + "/" + "channel_measurement_" + str(index_now).zfill(6) + '.npy')
    # print(test)
def create_dataset_by_GAN(scenario, train_IDs, labels, num,data_path):
    data_path_new = '/home/lxhit/code/CSI-position/data/new-data/mamimo_measurements/'
    data_path_s = data_path + "channel_measurements_" + scenario + "/"
    maxInRows = np.amax(labels, axis=0)
    minInRows = np.amin(labels, axis=0)
    x_min = minInRows[0]
    x_max = maxInRows[0]
    y_min = minInRows[1]
    y_max = maxInRows[1]
    num_flag = int(num)
    max_loop_num = int(num * 2)
    new_IDs = []
    while num_flag > 0 and max_loop_num > 0:
        label_x = random.randint(x_min, x_max)
        label_y = random.randint(y_min, y_max)
        label_new = np.array([label_x, label_y])
        if (labels == label_new).all(1).any():
            min_IDs = find_minimal_distance_IDS(label_new,train_IDs,labels,top_k=1000)
            csi_tuple = create_CSI_by_GAN(min_IDs,labels,data_path_s)
            X_new = np.reshape(csi_tuple, (64 , 100))
            #print(X_new)
            # add new train id to lables by adding sample_number
            labels = np.vstack((labels,label_new))
            index_now = labels.shape[0] -1
            # save the csi to the file
            save_csi_to_file(index_now, X_new, data_path_new,scenario)
            # update train_id
            new_IDs.append(index_now)
    ...
    create_num = int(num) - num_flag
    print("create dataset num: ")
    print(create_num)

    new_train_IDs = np.array(new_IDs)
    train_IDs_new = np.append(train_IDs,new_train_IDs)
    labels_new = labels
    return train_IDs_new, labels_new, data_path_new


num_samples = 252004

# Training size
trainings_size = 0.25                     # 25% training set
creating_size = 0.01                       # 60% create dataset by Using GAN
...
for scenario in scenarios:
    ...
    for num_antenna in num_antennas:
        ...
        # create dataset by using GAN
        num = creating_size* actual_num_samples
        train_IDs_new, labels_new, data_path_new = create_dataset_by_GAN(scenario, train_IDs, labels, num, data_path=data_path)

        train_generator = DataGenerator(scenario, train_IDs_new, labels_new,
