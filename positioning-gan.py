...
def create_dataset_by_GAN(scenario, train_IDs, labels,num, data_path):
    ...
    # get the train_IDS and associated data
    for ID in train_IDs:
        X = np.load(data_path_s + "channel_measurement_" + str(ID).zfill(6) + '.npy')
        y = labels[ID, :]
        print(X.shape)
        print(y.shape)
        merged_train_data = np.concatenate((X, y), axis=1)
        print(merged_train_data.shape)
        # gan = GANNumericalLabel()
        # gan.train(merged_train_data)
        # noise = np.random.normal(0, 1, (num, 10))
        # new_numerical_label_data = gan.generator.predict(noise)

    train_IDs_new = train_IDs
    labels_new = labels
    data_path_new = '/home/lxhit/code/CSI-position/data/new-data/dataset_GAN/'
